@extends('layouts.crm', ['title' => ($company->exists ? 'Edit company' : 'New company') . ' | Call CRM'])

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">{{ $company->exists ? 'Edit company' : 'New company' }}</h1>
        <p class="text-sm text-slate-600">MVP company form with basic validation.</p>
    </div>
    ...
    <form method="POST" action="{{ $company->exists ? route('companies.update', $company) : route('companies.store') }}" class="space-y-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('companies.index') }}" class="text-sm text-slate-600 hover:text-slate-900">Cancel</a>
        </div>
    </form>
@endsection
