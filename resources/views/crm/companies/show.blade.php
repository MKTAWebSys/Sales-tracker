    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-600">
                ...
                | Vlastnik: {{ $company->assignedUser?->name ?? '-' }}
            </p>
        </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 lg:col-span-2">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-2 text-sm">
                @if ($company->website)
                    <a href="{{ $company->website }}" target="_blank" rel="noreferrer" class="rounded-md bg-slate-100 px-3 py-1.5 text-slate-700 ring-1 ring-slate-200 hover:bg-slate-200">
                        Otevrit web
                    </a>
                @endif
                @if ($kurzyIcoUrl)
                    <a href="{{ $kurzyIcoUrl }}" target="_blank" rel="noreferrer" class="rounded-md bg-blue-50 px-3 py-1.5 text-blue-800 ring-1 ring-blue-200 hover:bg-blue-100">
                        Kurzy.cz podle ICO
                    </a>
                @endif
            </div>
        ...
        <div class="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <ul class="mt-4 space-y-3 text-sm text-slate-700">
                @forelse ($timeline as $index => $item)
                    ...
                    @endphp
                    <li
                        class="js-timeline-item rounded-lg border p-3 transition {{ $isLatest ? 'border-emerald-200 bg-emerald-50/40 shadow-sm ring-1 ring-emerald-100' : 'border-slate-100 bg-white' }} {{ $hasLongSummary ? 'cursor-pointer' : '' }}"
                        data-expanded="0"
                        data-collapsed-class="{{ $isLatest ? 'line-clamp-6' : 'line-clamp-3' }}"
                    >
                        @if ($isLatest)
                            <div class="mb-2 inline-flex items-center rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-medium text-emerald-800 ring-1 ring-emerald-200">
                                Posledni aktivita
                            </div>
                        @endif
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2">
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset {{ $typeBadge }}">
                                    {{ $item['type'] }}
                                </span>
                        ...
                        @if (!empty($item['summary']))
                            ...
                            @if ($hasLongSummary)
                                <div class="mt-2">
                                    <button type="button" class="js-timeline-toggle text-xs font-medium text-slate-600 underline hover:text-slate-900">
                                        Rozbalit poznamku
                                    </button>
                                </div>
                            @endif
